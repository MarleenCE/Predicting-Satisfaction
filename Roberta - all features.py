...
class CustomSequenceClassification(nn.Module):
    def __init__(self, num_labels=1, pretrained_model_name="pdelobelle/robbert-v2-dutch-base", num_numeric_features=4,  dropout_rate=0.0):
        super().__init__()
...
def prepare_data(data_df):
    ...
    additional_features = torch.tensor(data_df[["Travel Month", 'Travel Duration', 'Land', 'sentiment_score']].values, dtype=torch.float)
    
    # Convert labels to tensor
    labels = torch.tensor(data_df["Grade"].values, dtype=torch.float)  
    
    return TensorDataset(inputs_text.input_ids, inputs_text.attention_mask, additional_features, labels) 
