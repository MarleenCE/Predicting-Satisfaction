...
class CustomSequenceClassification(nn.Module):
    def __init__(self, num_labels=1, pretrained_model_name="pdelobelle/robbert-v2-dutch-base"):
        super().__init__()
        ...
        self.regression_head = nn.Linear(self.roberta.config.hidden_size, num_labels)
...
def prepare_data(data_df):
    ...
    return TensorDataset(inputs_text.input_ids, inputs_text.attention_mask, labels) 
...
model = CustomSequenceClassification(num_labels=1, pretrained_model_name="pdelobelle/robbert-v2-dutch-base")
...
directory = 'D:\\tempmarl'
best_model_path = os.path.join(directory, 'best_model_all.pth')
