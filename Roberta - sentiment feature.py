...
class CustomSequenceClassification(nn.Module):
    def __init__(self, num_labels=1, pretrained_model_name="pdelobelle/robbert-v2-dutch-base", num_numeric_features=1,  dropout_rate=0.0):
        super().__init__()
    ...
    def forward(self, input_ids, attention_mask, sentiment):
        ...
        combined_features = torch.cat((pooled_output, sentiment), dim=1)
...
def prepare_data(data_df):
    ...
    sentiment = torch.tensor(data_df["sentiment_score"].values, dtype=torch.float).unsqueeze(1)  
    
    # Convert labels to tensor
    labels = torch.tensor(data_df["Grade"].values, dtype=torch.float)  
    
    return TensorDataset(inputs_text.input_ids, inputs_text.attention_mask, sentiment, labels) 
...
directory = 'D:\\tempmarl'
best_model_path = os.path.join(directory, 'best_model_sentiment.pth')
...
for epoch in range(num_epochs):
    ...
    for batch in tqdm(train_loader):
        input_ids, attention_mask, sentiment, labels = [t.to(device) for t in batch] 
        optimizer.zero_grad()
    ...
    with torch.no_grad():
        for batch in validation_loader:  
            ...
            outputs = model(input_ids=input_ids, attention_mask=attention_mask, sentiment=sentiment)
